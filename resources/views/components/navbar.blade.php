<nav class="w-full fixed top-0 z-50 backdrop-blur-xl bg-white/70 shadow-lg border-b border-gray-200">
    <div class="max-w-screen-xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3">
            <div
                class="w-10 h-10 bg-gradient-to-r from-indigo-600 to-purple-500 rounded-full shadow-lg transform hover:scale-110 transition-all duration-300"></div>
            <span class="text-2xl font-bold text-gray-900 tracking-tight">Habita<span
                    class="text-indigo-600">X</span></span>
        </div>
        ...
        <div class="hidden md:flex items-center space-x-10">
            ...
            <a href="{{ url('/contact') }}"
               class="relative group text-gray-700 font-medium text-lg transition-all duration-300">
                Contact
                <span
                    class="absolute left-0 bottom-[-4px] w-0 h-[2px] bg-indigo-500 group-hover:w-full transition-all duration-300"></span>
            </a>
        ...
        <div class="hidden md:flex items-center space-x-5">
            @guest
                <a href="{{ route('login') }}"
                   class="text-gray-600 hover:text-indigo-600 text-md font-medium transition-all duration-300">
                    Sign In
                </a>
            @else
                ...
                <div id="profileDropdownWrapper" class="relative">
                    <button id="profileBtn" class="flex items-center space-x-2 focus:outline-none">
                        <img src="https://i.pravatar.cc/40?u={{ auth()->user()->id }}" class="w-8 h-8 rounded-full"
                             alt="User Avatar">
                        <span class="text-gray-800 font-semibold">{{ auth()->user()->name }}</span>
                    </button>

                    <div id="profileMenu"
                         class="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-md shadow-lg opacity-0 invisible transition-all duration-300">
                        <a href="{{ route('account') }}"
                           class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">My Profile</a>
                        <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Property
                            creation</a>
        ...
        <div id="mobileMenu" class="md:hidden px-6 pb-4 hidden flex-col gap-4 bg-white shadow-md">
            ...
            @guest
                <a href="{{ route('login') }}" class="text-gray-600 hover:text-indigo-600">Sign In</a>
            @else
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
            ...
            @endguest
            <a href="#" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-center">Publicar
                Anúncio</a>
        </div>
    </div>
</nav>
